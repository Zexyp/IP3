                            ...
                            <table class="table-auto w-full">
                                ...
                                @foreach($data as $value)
                                    <tr style="border-top: 1px solid #8888;">
                                        <td class="text-center"><a class="underline text-blue-500 hover:text-blue-700" href="{{route('incoming.view', [$value->id])}}">{{$value->date}}</td>
                                        ...
                                        @if(Auth::user()->role == Role::ADMIN)
                                            <td>
                                                <form action="{{route('incoming.delete', [$value->id])}}" method="post">
                                                    @csrf
                                                    @method('delete')
